...
- `left`, `right` -- Adjust the selection one cell to the left or right respectively.
- `up`, `down` -- Adjust the selection one line upwards or downwards respectively.
- `page_up`, `page_down` -- Adjust the selection one page upwards or downwards respectively.
- `home`, `end` -- Adjust the selection to the top-left or the bottom-right corner of the screen respectively.
- `beginning_of_line`, `end_of_line` -- Adjust the selection to the beginning or the end of the line respectively.
...
- `copy` -- Copy the file path into the clipboard.
...
- `right`, `down`, `left`, `up` -- Creates a new split in the corresponding direction.
- `auto` -- Creates a new split along the larger direction. For example, if the parent split is currently wider than it is tall, then a left-right split would be created, and vice versa.
...
Close the current tab and all splits therein or close all tabs and splits therein of tabs other than the current tab, depending on the mode.
...
As quick terminals are often useful when other windows are currently focused, they are best used with global keybinds. For example:
...
Not every action in Ghostty can be undone or redone. The list of actions that support undo/redo is currently limited to:
