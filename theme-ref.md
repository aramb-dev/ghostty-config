...
Ghostty ships with hundreds of built-in themes that can be selected with one line of configuration by using the `theme` configuration option. For example, to use the popular Catppuccin theme:

```
theme = Catppuccin Frappe
```

> **Tip:** Don't forget to reload your configuration after changing the theme.

The built-in themes are sourced from [iterm2-color-schemes](https://github.com/mbadolato/iTerm2-Color-Schemes) and updated in the Ghostty repository on the main branch weekly. If you want to contribute a new theme, please contribute it to iterm2-color-schemes and it will be automatically picked up by Ghostty.
...
> **Warning:** Themes can modify any configuration option, so be careful when using themes from untrusted sources. Always review the theme file before using it to ensure it doesn't contain malicious configuration.
