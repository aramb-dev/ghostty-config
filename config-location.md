...
> **Note:** In the future, we plan to also support native GUIs for configuration in line with our native UI philosophy. This will be in addition to the text-based configuration. Presently, the text-based configuration is the only way to configure Ghostty.
...
> **Note:** Some configuration options cannot be reloaded at runtime; others may only apply to newly created terminals. See the specific configuration option for details.
...
> **Tip:** The online reference documentation is available on the [Ghostty website](https://ghostty.org/docs/config/reference).
...
> **Note:** You may see strange looking blank configurations like `font-family =`. This is a valid syntax to specify the default behavior (no value). The `+show-config` outputs it so it's clear that key is defaulting and also to have something to attach the doc comment to.
